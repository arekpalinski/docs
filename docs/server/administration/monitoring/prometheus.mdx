...
* In this page:  
   ...
   * [RavenDB Prometheus Endpoint](../../../server/administration/monitoring/prometheus.mdx#ravendb-prometheus-endpoint)  
       ...
       * [Metrics Provided by the Prometheus Endpoint](../../../server/administration/monitoring/prometheus.mdx#metrics-provided-by-the-prometheus-endpoint)  
           * [GC Metrics](../../../server/administration/monitoring/prometheus.mdx#gc-metrics)  
...
Some metrics are **not included by default** and must be explicitly requested using opt-in query parameters:  

| Parameter | Default | Description |
| - | - | - |
| `includeGcMetrics` | `false` | Set to `true` to include [GC metrics](../../../server/administration/monitoring/prometheus.mdx#gc-metrics) in the response. |

E.g., to include GC metrics use -  
http://localhost:8080/admin/monitoring/v1/prometheus?includeGcMetrics=true
...
GC metrics are included in the Prometheus output only when the `includeGcMetrics=true` query parameter is set.  
All GC metrics carry a `gckind` label (currently always `any`).

| Metric | Description |
| - | - |
| gc_compacted | Whether the last garbage collection was compacting (`1` = true, `0` = false) |
| gc_concurrent | Whether the last garbage collection was concurrent (`1` = true, `0` = false) |
...
| gc_pause_durations1_seconds | First GC pause duration in seconds recorded during the last garbage collection |
| gc_pause_durations2_seconds | Second GC pause duration in seconds recorded during the last garbage collection |
